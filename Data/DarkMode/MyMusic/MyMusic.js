...
WS.onmessage = (event) => {
   ...
   if(event.data=="UpdatedMusic"){
       ...
       if(JSON.parse(event.data)[0].action=="MusicList"){
   ...
   playlist.forEach(song => {
       ...
       li.innerHTML = `
           <a href="#" id="${song.url}">
               ...
               <div class="time" id=${song.Lyric.replace(" ","")}>
...
function Remove(el){
   ...
   console.log(nameD);
   ws.send(JSON.stringify({action:"RemoveM",name:nameD}));
}
var type=21;
var fc = document.querySelector(".fc");
fc.addEventListener("input", (event) => {
       Filt(type,fc.value);
});
...
function switchPlaylistType(type) {
   ...
   if (type === 'local') {
       updatePlaylist(localPlaylist);
       let tmp=window.top.GetNow();
       document.querySelectorAll(`#${tmp}`).classList.add('playing');
...
function UpdateFilter(){
   ...
   Filt(11, document.querySelector(".Fil").value);
}
...
document.addEventListener('click', e => {
   if (e.target.classList.contains('de')) {
       ...
       const songData = {
           ...
           album: li.querySelector('.Album').id,
           albumArt: li.querySelector('.album-cover').src,
           filePath:li.querySelector('.time').id,
           tags:tmp
       };
       Notrrans=0;
...
function showMusicEditor(songData){
   ...
   document.querySelector('#artist').value=songData.artist;
   document.querySelector('#album').value=songData.album;
   document.querySelector('#file-path').value = songData.filePath;
...
tagsContainer.addEventListener('click', e => {
   if(e.target.closest('#add-tag')) {
       if(tags.length >= 2) return window.top.Alert('最多只能添加2个标签');
...
function submitData() {
   ...
   var Url=document.querySelector('.bian-ji #file-path').value.replace(/\\/g,"/");
   const fullData = {
       action: "UpdateMeta",
       timestamp: Date.now(),
       Meta: {
           ...
           singer: document.querySelector('.bian-ji #artist').value,
           album: document.querySelector('.bian-ji #album').value,
           filePath: Url,
           pic:base64Data,
           tags: tags,
       }
...
function submitDataO() {
   ...
   const fullData = {
       ...
       Meta: {
           prename:Name,
           name: document.querySelector('.editor-container #song-name').value,
           singer: document.querySelector('.editor-container #artist').value,
           album: "404",
           pic:document.querySelector(".OnlinePic").src,
           tags: " ",
       }
   };
   document.querySelector(".Loc").innerHTML="提交中";
...
function OnlineSearch() {
    let Type = document.querySelector(".SOn").value;
    console.log(Type);
    var id = document.querySelector(".editor-container").querySelector("#song-id").value;

    if (isNaN(id) && !id.trim() !== '' && Type == "网易云音乐") {
        document.querySelector(".editor-container").querySelector("#song-name").value = "id错误请修改";
        document.querySelector(".editor-container").querySelector("#artist").value = "id错误请修改";
        document.querySelector(".editor-container").querySelector("#Condition").value = "id错误请修改";
        return;
    }

    document.querySelector(".editor-container").querySelector("#search-online").disabled = true;
    document.querySelector(".editor-container").querySelector("#search-online").innerHTML = "正在搜索...";

    // 开始异步操作链
    sendApiRequestWithInterval('song', id, Type, function(result) {
        document.querySelector(".editor-container").querySelector("#search-online").disabled = false;
        document.querySelector(".editor-container").querySelector("#search-online").innerHTML = "查询在线数据";
        console.log("Unlock");

        if (result == null || result.length == 0 || result.error == "unknown song") {
            document.querySelector(".editor-container").querySelector("#song-name").value = "未找到歌曲";
            document.querySelector(".editor-container").querySelector("#artist").value = "未找到歌曲";
            document.querySelector(".editor-container").querySelector("#Condition").value = "未找到歌曲";
            return;
        }
        ...
        document.querySelector(".editor-container").querySelector("#song-name").value = item.name;
        document.querySelector(".editor-container").querySelector("#artist").value = item.artist;

        if (item.lrc != null) {
            document.querySelector(".editor-container").querySelector("#Condition").value = "已找到";
            downloadLrcAsTextWithInterval(item.lrc, function(lrcText) {
                Lyric = lrcText;
                ws.send(JSON.stringify({action: "Lyric", Lyric: Lyric, Name: Name}));
            });
        }

        if (!(Type == "网易云音乐")) {
            document.querySelector(".OnlinePic").src = "../BackDrop/UnKnD.png";
            return;
        }

        if (item.pic) {
            fetchImageWithInterval(item.pic, function(base64Data) {
                if (base64Data) {
                    document.querySelector(".OnlinePic").src = base64Data;
                } else {
                    document.querySelector(".OnlinePic").src = "../BackDrop/UnKnD.png";
                }
            });
...
// 使用 setInterval 模拟异步请求
function sendApiRequestWithInterval(type, id, webside, callback) {
    const baseUrl = 'https://api.injahow.cn/meting/';
    var queryParams;
    if (webside == "网易云音乐") {
        queryParams = `?type=${type}&id=${id}`;
    } else {
        queryParams = `?type=${type}&id=${id}&server=tencent`;
    }
    ...
    fetch(url)
        ...
        .catch(err => {
            error = err;
            requestDone = true;
            window.top.Alert('请求过程中出现错误:', error);
        });

    // 使用 setInterval 检查请求状态
...
// 使用 setInterval 模拟下载歌词
function downloadLrcAsTextWithInterval(url, callback) {
    let requestDone = false;
    let lrcText = null;
    let error = null;
    ...
    fetch(url)
        ...
        .catch(err => {
            error = err;
            requestDone = true;
            window.top.Alert('下载歌词时出现错误:', error);
        });
...
// 使用 setInterval 模拟图片下载
function fetchImageWithInterval(url, callback) {
    let requestDone = false;
    let base64Data = null;
    let error = null;
