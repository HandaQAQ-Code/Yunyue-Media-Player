...
WS.onmessage = (event) => {
    if(event.data=="Updated" || event.data=="DeletedeM"){
        ...
        if(JSON.parse(event.data)[0].action=="MusicLists"){
...
// 渲染歌单列表
 function renderPlaylists() {
    ...
    playlists.forEach(playlist => {
        ...
        if(playlist.name=="播放历史" || playlist.name=="喜欢的音乐"){
            li.innerHTML = `
            <a href="#">
...
document.querySelector('.confirm-rename').addEventListener('click', () => {
    ...
    document.querySelectorAll(".music-name").forEach(el => {
        if(el.innerHTML==newName){
            document.querySelector(".confirm-create").innerHTML="歌单名已存在";
        setTimeout(() => {
            document.querySelector(".confirm-create").innerHTML="创建歌单";
        }, 1000);
        return;
        }
    })
...
document.querySelector('.playlist-page .back-btn').addEventListener('click', () => {
 ...
 function showPlaylistPage(playlistId) {
    ...
    document.getElementById('playlist-count').textContent = `共${playlist.songCount}首`;
...
function Submit(){
    ...
    if(value==" " || value=="" || value==null){
        document.querySelector(".confirm-create").innerHTML="请输入歌单名";
        setTimeout(() => {
            document.querySelector(".confirm-create").innerHTML="创建歌单";
        }, 1000);
        return;
...
function PlayAll(el){
    window.top.Loading();
    window.top.PlayAllLM(el.parentElement.querySelector("#playlist-title").textContent);
  }
function SubmitR(el,bl){
 ...
 function showPlaylistPage(playlistId) {
    ...
    // 更新播放列表信息
    document.getElementById('playlist-title').textContent = playlist.name;
    document.getElementById('playlist-count').textContent = `共${playlist.songs.length}首`;
        ...
        <li data-song-id="${song.id}">
            <a href="#" id="${song.url}">
                ...
                <span class="song-info">
                    ...
                    <p class="song-artist">${song.artist?song.artist:'未知歌手'}</p>
...
function bindSongEvents() {
    ...
    songsList.querySelectorAll('.delete-song-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            WS.send(JSON.stringify({"action":"ReMoveL","name":this.closest('li').querySelector("a").id,"List":document.querySelector("#playlist-title").innerHTML}));
...
function playSong(element) {
    if(Occ==false){
        ...
        console.log(element.querySelector(".song-title").innerHTML);
